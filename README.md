...
| `vgm`   | [vgmrips.net](https://vgmrips.net/) | VGM music packs for the chips pico-spec can drive (`SAA1099`, `SN76489`, `YM2413`, `YM3812`, `YMF262`), as `<chip>/<pack>/<track>.vgm`. HTML scrape of the per-chip pack listings ([packs/chips](https://vgmrips.net/packs/chips)); dual-chip rips ("2xSN76489" etc.) have no category of their own upstream and appear inside the base chip's dir, and same-die card tags (AD-Lib/Sound Blaster/OPLL variants) are unioned in. Track links on the pack page are direct per-track `.vgz` (gzipped `.vgm`) — the device has no gunzip, so entries carry no direct link: the exporter/server downloads and **gunzips to a ready `.vgm`** at build/request time. The site sits behind the Anubis anti-bot wall, which challenges browser UAs but passes honest bot UAs — this adapter is the one scraper here that must NOT pretend to be a browser |
